  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      <p class="item-desc">Description: ${item.description}</p>
       <button onclick="increase('${doc.id}')" class="btn btn-success">+</button>
              <p id="count-${doc.id}" class="d-flex justify-content-center">${productCounts[doc.id]}</p>
              <button onclick="decrease('${doc.id}')" class="btn btn-danger">-</button>
      </div>
      </div>
      `;
    });
...
async function decrease(id, productId) {
